...
---Allow player to respawn by holding E.
---Auto-respawns when DeathTime reaches 0.
function CheckForRespawn()
    local holdStartedAt
    local lastTick = GetGameTimer()

    while DeathState == sharedConfig.deathState.DEAD do
        -- Decrement DeathTime every 1000 ms
        if GetGameTimer() - lastTick >= 1000 then
        ...
        if allowRespawn then
            if IsControlPressed(0, 38) then
                ...
                if GetGameTimer() - holdStartedAt >= 5000 then
                    respawn()
                    return
                end
            else
