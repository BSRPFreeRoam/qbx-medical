...
--- low level GTA resurrection — saves and restores outfit because NetworkResurrectLocalPlayer resets ped appearance.
function ResurrectPlayer()
    local saved = SavePedClothes(cache.ped)
    ...
    RestorePedClothes(cache.ped, saved)
end
...
function StartLastStand(attacker, weapon)
    ...
    CreateThread(function()
        while DeathState == sharedConfig.deathState.LAST_STAND do
            countdownLastStand()
            UpdateMedicalUi('critical', LaststandTime)
            Wait(1000)
        end
    end)
